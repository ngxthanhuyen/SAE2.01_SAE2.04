...
db = DataBase()
#####################################################################
# FONCTIONS AUXILIAIRES POUR RÉCUPÉRATION DES ID
#####################################################################
def get_id_commune(code_commune_insee):
   query = "SELECT id_commune FROM COMMUNES WHERE code_commune_insee = ?"
   db.cursor.execute(query,(code_commune_insee,))
   result = db.cursor.fetchone()
   return result[0] if result else None

def get_id_departement(code_departement):
   query = "SELECT id_departement FROM DEPARTEMENTS WHERE code_departement = ?"
   db.cursor.execute(query,(code_departement,))
   result = db.cursor.fetchone()
   return result[0] if result else None

def get_id_masse_eau(code_masse_eau):
   query = "SELECT id_masse_eau FROM MASSE_EAU WHERE code_masse_eau_edl = ?"
   db.cursor.execute(query, (code_masse_eau,))
   result = db.cursor.fetchone()
    ...
    if response.status_code == 200 or response.status_code == 206:
        ...
        if 'data' in data:
            ...
            for entry in data['data']:
              ...
              db.insert_commune(entry['code_commune_insee'], entry['nom_commune'])
              ...
              if code_masse_eau and nom_masse_eau and urn_masse_eau:
              ...
              if id_commune and id_departement and id_masse_eau:
                db.insert_station(id_commune, id_departement, id_masse_eau, entry['code_bss'], entry['bss_id'], entry['urn_bss'], entry['profondeur_investigation'], entry['x'], entry['y'], entry['altitude_station'], entry['nb_mesures_piezo'])           
